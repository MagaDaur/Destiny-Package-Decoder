// ---- Created with 3Dmigoto v1.3.16 on Fri Jun 30 00:18:41 2023
Buffer<uint4> t1 : register(t1);

Buffer<float4> t0 : register(t0);

cbuffer cb1 : register(b1)
{
  float4 cb1[24];
}

cbuffer cb12 : register(b12)
{
  float4 cb12[16];
}

cbuffer cb0 : register(b0)
{
  float4 cb0[5];
}




// 3Dmigoto declarations
#define cmp -


void main(
  float4 v0 : POSITION0,
  float4 v1 : NORMAL0,
  float4 v2 : TANGENT0,
  float2 v3 : TEXCOORD0,
  uint v4 : SV_VERTEXID0,
  out float4 o0 : TEXCOORD0,
  out float4 o1 : TEXCOORD1,
  out float4 o2 : TEXCOORD2,
  out float4 o3 : TEXCOORD3,
  out float4 o4 : TEXCOORD4,
  out float4 o5 : TEXCOORD8,
  out float4 o6 : SV_POSITION0)
{
// Needs manual fix for instruction:
// unknown dcl_: dcl_input_sgv v4.x, vertex_id
  float4 r0,r1,r2,r3,r4,r5,r6,r7,r8,r9,r10,r11,r12,r13,r14;
  uint4 bitmask, uiDest;
  float4 fDest;

  r0.x = min(asuint(cb1[4].w), (uint)v4.x);
  r0.xyzw = t0.Load(r0.x).xyzw;
  r1.x = cb0[0].x * r0.x;
  r1.y = cb0[1].y * r0.x + cb0[1].x;
  r1.y = cb0[2].x + r1.y;
  r1.z = cb0[3].w * r1.y + cb0[3].z;
  r1.z = 0.25 + r1.z;
  r1.w = round(r1.z);
  r1.z = r1.z + -r1.w;
  r1.w = abs(r1.z) * -16 + 8;
  r1.z = r1.z * r1.w;
  r1.w = abs(r1.z) * 0.224999994 + 0.774999976;
  r1.z = r1.z * r1.w;
  r2.x = cb0[3].x * r1.z + cb0[3].y;
  r1.y = cb0[4].w * r1.y + cb0[4].z;
  r1.y = 0.25 + r1.y;
  r1.z = round(r1.y);
  r1.y = r1.y + -r1.z;
  r1.z = abs(r1.y) * -16 + 8;
  r1.y = r1.y * r1.z;
  r1.z = abs(r1.y) * 0.224999994 + 0.774999976;
  r1.y = r1.y * r1.z;
  r2.y = cb0[4].x * r1.y + cb0[4].y;
  r2.z = 0;
  r1.y = 32767.0996 * v0.w;
  r1.y = (int)r1.y;
  r1.z = max((int)r1.y, (int)-r1.y);
  r1.w = cmp((int)r1.z >= 2048);
  r2.w = cmp((int)r1.y < 0);
  r3.x = (uint)v4.x << 1;
  r3.x = r2.w ? r3.x : v4.x;
  r1.z = (uint)r1.z << 3;
  r1.z = (int)r1.z + 0xffffc000;
  bitmask.z = ((~(-1 << 3)) << 0) & 0xffffffff;  r1.z = (((uint)r3.x << 0) & bitmask.z) | ((uint)r1.z & ~bitmask.z);
  r3.xyzw = t1.Load(r1.z).xyzw;
  r1.y = r1.w ? r3.x : r1.y;
  r3.x = (int)r1.y * 3;
  r4.xy = mad(int2(3,3), (int2)r1.yy, int2(1,2));
  if (r1.w != 0) {
    r1.yw = (uint2)r3.zw;
    r1.yw = float2(0.00392156886,0.00392156886) * r1.yw;
    r5.x = cb1[r3.x+8].w;
    r5.y = cb1[r4.x+8].w;
    r5.z = cb1[r4.y+8].w;
    r3.z = (int)r3.y * 3;
    r3.yw = mad(int2(3,3), (int2)r3.yy, int2(1,2));
    r6.xyz = cb1[r3.z+8].xyz * r1.www;
    r7.x = cb1[r3.z+8].w * r1.w;
    r7.y = cb1[r3.y+8].w * r1.w;
    r7.z = cb1[r3.w+8].w * r1.w;
    r8.xyz = cb1[r3.y+8].xyz * r1.www;
    r3.yzw = cb1[r3.w+8].xyz * r1.www;
    r6.xyz = cb1[r3.x+8].xyz * r1.yyy + r6.xyz;
    r5.xyz = r5.xyz * r1.yyy + r7.xyz;
    r7.xyz = cb1[r4.x+8].xyz * r1.yyy + r8.xyz;
    r8.xyz = cb1[r4.y+8].xyz * r1.yyy + r3.yzw;
    if (r2.w != 0) {
      r1.y = (int)r1.z + 1;
      r9.xyzw = t1.Load(r1.y).xyzw;
      r1.yz = (uint2)r9.zw;
      r1.yz = float2(0.00392156886,0.00392156886) * r1.yz;
      r3.yz = (int2)r9.xy * int2(3,3);
      r9.xyzw = mad(int4(3,3,3,3), (int4)r9.xxyy, int4(1,2,1,2));
      r10.x = cb1[r3.y+8].w * r1.y;
      r10.y = cb1[r9.x+8].w * r1.y;
      r10.z = cb1[r9.y+8].w * r1.y;
      r11.xyz = r1.yyy * cb1[r3.y+8].xyz + r6.xyz;
      r10.xyz = r10.xyz + r5.xyz;
      r12.xyz = r1.yyy * cb1[r9.x+8].xyz + r7.xyz;
      r13.xyz = r1.yyy * cb1[r9.y+8].xyz + r8.xyz;
      r14.x = cb1[r3.z+8].w * r1.z;
      r14.y = cb1[r9.z+8].w * r1.z;
      r14.z = cb1[r9.w+8].w * r1.z;
      r6.xyz = r1.zzz * cb1[r3.z+8].xyz + r11.xyz;
      r5.xyz = r14.xyz + r10.xyz;
      r7.xyz = r1.zzz * cb1[r9.z+8].xyz + r12.xyz;
      r8.xyz = r1.zzz * cb1[r9.w+8].xyz + r13.xyz;
    }
  } else {
    r6.xyz = cb1[r3.x+8].xyz;
    r5.x = cb1[r3.x+8].w;
    r5.y = cb1[r4.x+8].w;
    r5.z = cb1[r4.y+8].w;
    r7.xyz = cb1[r4.x+8].xyz;
    r8.xyz = cb1[r4.y+8].xyz;
  }
  r1.yzw = v0.xyz * cb1[5].www + cb1[5].xyz;
  r1.xyz = r1.xxx * r2.xyz + r1.yzw;
  r2.xyz = cb12[15].xyz + r5.xyz;
  r6.w = r2.x;
  r1.w = 1;
  r3.x = dot(r6.xyzw, r1.xyzw);
  r7.w = r2.y;
  r3.y = dot(r7.xyzw, r1.xyzw);
  r8.w = r2.z;
  r3.z = dot(r8.xyzw, r1.xyzw);
  r2.xyzw = cb12[1].xyzw * r3.yyyy;
  r2.xyzw = cb12[0].xyzw * r3.xxxx + r2.xyzw;
  r2.xyzw = cb12[2].xyzw * r3.zzzz + r2.xyzw;
  o6.xyzw = cb12[14].xyzw + r2.xyzw;
  o4.xyz = cb12[7].xyz + r3.xyz;
  r1.x = dot(v1.xyz, v1.xyz);
  r1.x = rsqrt(r1.x);
  r1.xyw = v1.xyz * r1.xxx;
  r2.x = dot(r6.xyz, r1.xyw);
  r2.y = dot(r7.xyz, r1.xyw);
  r2.z = dot(r8.xyz, r1.xyw);
  r1.x = dot(v2.xyz, v2.xyz);
  r1.x = rsqrt(r1.x);
  r1.xyw = v2.xyz * r1.xxx;
  r3.x = dot(r6.xyz, r1.xyw);
  r3.y = dot(r7.xyz, r1.xyw);
  r3.z = dot(r8.xyz, r1.xyw);
  r1.xyw = r3.yzx * r2.zxy;
  r1.xyw = r2.yzx * r3.zxy + -r1.xyw;
  o2.xyz = v2.www * r1.xyw;
  r1.xy = v3.xy * cb1[6].xy + cb1[6].zw;
  r1.w = saturate(cb1[7].z * r2.z);
  o0.w = saturate(cb1[7].w + r1.w);
  r1.w = 32767.0996 * v1.w;
  r1.w = (int)r1.w;
  r2.w = (uint)r1.w >> 3;
  r2.w = (int)r2.w & 4095;
  r1.w = (int)r1.w & 7;
  r1.w = (int)r1.w;
  o2.w = 0.100000001 + r1.w;
  r4.xyzw = t1.Load(r2.w).xyzw;
  r4.yw = (uint2)r4.yw << int2(8,8);
  r4.xy = (int2)r4.yw | (int2)r4.xz;
  r4.xy = f16tof32(r4.xy);
  o3.zw = r4.xy * r1.xy;
  o0.xyz = r2.xyz;
  o1.w = r1.z;
  o1.xyz = r3.xyz;
  o3.xy = r1.xy;
  o5.xyzw = r0.xyzw;
  return;
}