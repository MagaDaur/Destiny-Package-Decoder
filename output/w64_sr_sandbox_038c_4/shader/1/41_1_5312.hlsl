// ---- Created with 3Dmigoto v1.3.16 on Mon Jul 03 19:23:03 2023
Buffer<float4> t2 : register(t2);

cbuffer cb1 : register(b1)
{
  float4 cb1[4];
}

cbuffer cb12 : register(b12)
{
  float4 cb12[8];
}

cbuffer cb0 : register(b0)
{
  float4 cb0[124];
}




// 3Dmigoto declarations
#define cmp -


void main(
  float4 v0 : TEXCOORD0,
  float4 v1 : TEXCOORD1,
  float4 v2 : TEXCOORD2,
  float4 v3 : TEXCOORD3,
  uint v4 : SV_VERTEXID0,
  uint v5 : SV_InstanceID0,
  out float4 o0 : TEXCOORD0,
  out float4 o1 : TEXCOORD1,
  out float4 o2 : TEXCOORD2,
  out float4 o3 : TEXCOORD3,
  out float4 o4 : SV_POSITION0)
{
// Needs manual fix for instruction:
// unknown dcl_: dcl_input_sgv v4.x, vertex_id
  float4 r0,r1,r2,r3,r4,r5;
  uint4 bitmask, uiDest;
  float4 fDest;

  float4 x0[4];
  r0.x = (int)v4.x;
  r0.x = 0.5 + r0.x;
  r0.x = r0.x * 0.5 + -0.5;
  r0.x = round(r0.x);
  r0.y = (int)r0.x;
  r0.zw = (uint2)r0.yy << int2(1,2);
  r0.z = (int)-r0.z + (int)v4.x;
  r0.w = (int)r0.w;
  r0.w = cb0[36].x + r0.w;
  r1.x = (uint)r0.w;
  r1.x = t2.Load(r1.x).w;
  r1.yz = float2(1,3) + r0.ww;
  r1.yz = (uint2)r1.yz;
  r2.xyzw = t2.Load(r1.y).xyzw;
  r1.yz = t2.Load(r1.z).yz;
  x0[0].y = 0;
  x0[1].y = 1;
  x0[2].y = 0;
  x0[3].y = 1;
  r3.xyz = cb1[1].xyz * r2.yyy;
  r3.xyz = cb1[0].xyz * r2.xxx + r3.xyz;
  r3.xyz = cb1[2].xyz * r2.zzz + r3.xyz;
  r3.xyz = cb1[3].xyz + r3.xyz;
  r0.w = cmp(1 < cb0[120].x);
  if (r0.w != 0) {
    r0.w = cmp((int)r0.y < 0);
    if (r0.w != 0) {
      r0.w = cb0[36].x + 1;
      r0.w = (uint)r0.w;
      r4.xyz = t2.Load(r0.w).xyz;
    } else {
      r0.w = cb0[120].x + -1;
      r0.x = r0.x + -r0.w;
      r0.x = cmp(abs(r0.x) < 0.00100000005);
      if (r0.x != 0) {
        r0.x = (int)r0.y + -1;
        r0.x = (int)r0.x;
        r0.x = r0.x * cb0[37].x + cb0[36].x;
        r0.x = 1 + r0.x;
        r0.x = (uint)r0.x;
        r5.xyz = t2.Load(r0.x).xyz;
        r4.xyz = r2.xyz * float3(2,2,2) + -r5.xyz;
      } else {
        r0.x = (int)r0.y + 1;
        r0.x = (int)r0.x;
        r0.x = r0.x * cb0[37].x + cb0[36].x;
        r0.x = 1 + r0.x;
        r0.x = (uint)r0.x;
        r4.xyz = t2.Load(r0.x).xyz;
      }
    }
  } else {
    r4.xyz = float3(0,0,0);
  }
  r0.xyw = cb1[1].zxy * r4.yyy;
  r0.xyw = cb1[0].zxy * r4.xxx + r0.xyw;
  r0.xyw = cb1[2].zxy * r4.zzz + r0.xyw;
  r0.xyw = cb1[3].zxy + r0.xyw;
  r0.xyw = r0.xyw + -r3.zxy;
  r1.w = dot(r0.xyw, r0.xyw);
  r1.w = sqrt(r1.w);
  r1.w = cmp(9.99999975e-006 < r1.w);
  r0.xyw = r1.www ? r0.xyw : float3(0,1,0);
  r1.w = dot(r0.xyw, r0.xyw);
  r1.w = rsqrt(r1.w);
  r2.xyz = r1.www * r0.ywx;
  r4.xyz = cb12[7].xyz + -r3.xyz;
  r5.xyz = r4.yzx * r0.xyw;
  r0.xyw = r0.wxy * r4.zxy + -r5.xyz;
  r1.w = dot(r0.xyw, r0.xyw);
  r1.w = rsqrt(r1.w);
  r0.xyw = r1.www * r0.xyw;
  r5.xyz = float3(0.300000012,0.300000012,0.300000012) * r0.xyw;
  r1.w = (int)r0.z;
  r1.w = cmp(r1.w < 0.5);
  r0.xyw = float3(-0.300000012,-0.300000012,-0.300000012) * r0.xyw;
  r0.xyw = r1.www ? r0.xyw : r5.xyz;
  r3.xyz = r3.xyz + r0.xyw;
  r1.w = dot(r2.xyz, r4.xyz);
  r2.xyz = -r1.www * r2.xyz + r4.xyz;
  r1.w = dot(r2.xyz, r2.xyz);
  r1.w = rsqrt(r1.w);
  r2.xyz = r2.xyz * r1.www;
  r1.w = dot(r0.xyw, r0.xyw);
  r1.w = rsqrt(r1.w);
  r0.xyw = r0.xyw * r1.www + -r2.xyz;
  o1.xyz = r0.xyw * float3(0.800000012,0.800000012,0.800000012) + r2.xyz;
  o0.y = x0[r0.z+0].y;
  r0.xyzw = cb12[1].xyzw * r3.yyyy;
  r0.xyzw = cb12[0].xyzw * r3.xxxx + r0.xyzw;
  r0.xyzw = cb12[2].xyzw * r3.zzzz + r0.xyzw;
  r0.xyzw = cb12[3].xyzw + r0.xyzw;
  r1.w = cb0[123].x * cb0[16].y;
  r2.x = cmp(0.999000013 < r2.w);
  o0.z = r2.x ? 0 : r1.w;
  o2.xyz = cb12[7].xyz + -r3.xyz;
  o0.w = r0.w;
  o0.x = r1.x;
  o3.y = 0;
  o3.xzw = r1.xyz;
  o4.xyzw = r0.xyzw;
  return;
}