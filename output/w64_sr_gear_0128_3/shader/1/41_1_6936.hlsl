// ---- Created with 3Dmigoto v1.3.16 on Fri Jun 30 00:18:38 2023
cbuffer cb1 : register(b1)
{
  float4 cb1[7];
}

cbuffer cb12 : register(b12)
{
  float4 cb12[8];
}

cbuffer cb0 : register(b0)
{
  float4 cb0[114];
}




// 3Dmigoto declarations
#define cmp -


void main(
  float4 v0 : TEXCOORD0,
  float4 v1 : TEXCOORD1,
  float4 v2 : TEXCOORD2,
  float4 v3 : TEXCOORD3,
  float4 v4 : TEXCOORD4,
  float3 v5 : POSITION0,
  float2 v6 : POSITION1,
  float3 v7 : POSITION2,
  uint v8 : SV_VERTEXID0,
  uint v9 : SV_InstanceID0,
  out float4 o0 : TEXCOORD0,
  out float3 o1 : TEXCOORD1,
  out float p1 : TEXCOORD5,
  out float4 o2 : TEXCOORD2,
  out float4 o3 : TEXCOORD3,
  out float4 o4 : TEXCOORD4,
  out float4 o5 : SV_POSITION0)
{
  float4 r0,r1,r2,r3,r4,r5,r6,r7,r8,r9,r10,r11;
  uint4 bitmask, uiDest;
  float4 fDest;

  r0.z = cb1[0].y;
  r0.x = cb1[1].y;
  r0.y = cb1[2].y;
  r1.xyz = cb1[1].xyz * v2.yyy;
  r1.xyz = cb1[0].xyz * v2.xxx + r1.xyz;
  r1.xyz = cb1[2].xyz * v2.zzz + r1.xyz;
  r1.xyz = cb1[3].xyz + r1.xyz;
  r1.xyz = cb12[7].xyz + -r1.xyz;
  r0.w = dot(r1.xyz, r1.xyz);
  r0.w = rsqrt(r0.w);
  r1.xyz = r1.xyz * r0.www;
  r2.y = dot(r1.xyz, cb1[0].xyz);
  r2.z = dot(r1.xyz, cb1[1].xyz);
  r2.x = dot(r1.xyz, cb1[2].xyz);
  r1.y = cb1[1].z;
  r1.z = cb1[2].z;
  r1.x = cb1[0].z;
  r3.xyz = r1.xyz * r2.xyz;
  r1.xyz = r1.zxy * r2.yzx + -r3.xyz;
  r0.w = dot(r1.xyz, r1.xyz);
  r1.w = rsqrt(r0.w);
  r0.w = cmp(0.00100000005 < r0.w);
  r1.xyz = r1.xyz * r1.www;
  r0.xyz = r0.www ? r1.xyz : r0.xyz;
  r1.xyz = r2.xyz * r0.xyz;
  r1.xyz = r2.zxy * r0.yzx + -r1.xyz;
  r3.x = r2.y;
  r4.x = r2.z;
  r3.z = r1.x;
  r3.y = r0.z;
  r5.w = 0;
  r6.xyz = v5.xyz * cb1[4].xyz + cb1[5].xyz;
  r0.z = dot(cb0[110].xyz, cb0[110].xyz);
  r0.z = rsqrt(r0.z);
  r7.xyz = cb0[110].xyz * r0.zzz;
  r0.zw = float2(3.14100003,0.5) * v3.wy;
  sincos(r0.z, r1.x, r8.x);
  sincos(r0.w, r9.x, r10.x);
  r7.xyz = r1.xxx * r7.xyz;
  r8.yzw = -r7.yzx * r6.zxy;
  r11.xyz = r6.yzx * -r7.zxy + -r8.yzw;
  r5.xyz = r8.xxx * r6.xyz;
  r0.z = dot(r6.xyz, -r7.xyz);
  r11.w = -r0.z;
  r5.xyzw = r11.xyzw + r5.xyzw;
  r6.xyz = r7.zxy * r5.yzx;
  r6.xyz = r7.yzx * r5.zxy + -r6.xyz;
  r5.xyz = r8.xxx * r5.xyz + r6.xyz;
  r5.xyz = r5.www * r7.xyz + r5.xyz;
  r5.xyz = v4.xxx * r5.xyz;
  r6.x = dot(r5.xyz, r3.xyz);
  r4.y = r0.x;
  r2.y = r0.y;
  r4.z = r1.y;
  r2.z = r1.z;
  r6.y = dot(r5.xyz, r4.xyz);
  r6.z = dot(r5.xyz, r2.xyz);
  r0.xyz = r10.xxx * r6.xyz;
  r0.w = 0;
  r1.xyz = float3(1,0,0) * r9.xxx;
  r5.xyzw = r6.yzxx * -r1.zxzx;
  r6.xyz = -r6.zxy * -r1.zzx + r5.xyz;
  r6.w = -r5.w;
  r0.xyzw = r6.xyzw + r0.xyzw;
  r5.xyz = r1.zxz * r0.yzx;
  r5.xyz = r1.zzx * r0.zxy + -r5.xyz;
  r0.xyz = r10.xxx * r0.xyz + r5.xyz;
  r0.xyz = r0.www * r1.xzz + r0.xyz;
  r0.xyz = v2.xyz + r0.xyz;
  r5.xyz = cb1[1].xyz * r0.yyy;
  r0.xyw = cb1[0].xyz * r0.xxx + r5.xyz;
  r0.xyz = cb1[2].xyz * r0.zzz + r0.xyw;
  r0.xyz = cb1[3].xyz + r0.xyz;
  r5.xyzw = cb12[1].xyzw * r0.yyyy;
  r5.xyzw = cb12[0].xyzw * r0.xxxx + r5.xyzw;
  r5.xyzw = cb12[2].xyzw * r0.zzzz + r5.xyzw;
  o2.xyz = cb12[7].xyz + -r0.xyz;
  r0.xyzw = cb12[3].xyzw + r5.xyzw;
  o0.w = r0.w;
  r1.w = cb0[113].x * cb0[12].y;
  r2.w = cmp(0.999000013 < v2.w);
  r1.w = r2.w ? 0 : r1.w;
  o0.z = r1.w;
  r1.w = cmp(r1.w < 9.99999997e-007);
  o0.xy = v6.xy * cb1[6].xy + cb1[6].zw;
  r5.xyz = v7.xyz;
  r5.w = 0;
  r6.xyz = v7.zxy * -r7.yzx;
  r6.xyz = v7.yzx * -r7.zxy + -r6.xyz;
  r2.w = dot(v7.xyz, -r7.xyz);
  r6.w = -r2.w;
  r5.xyzw = r8.xxxx * r5.xyzw + r6.xyzw;
  r6.xyz = r7.zxy * r5.yzx;
  r6.xyz = r7.yzx * r5.zxy + -r6.xyz;
  r5.xyz = r8.xxx * r5.xyz + r6.xyz;
  r5.xyz = r5.www * r7.xyz + r5.xyz;
  r3.x = dot(r5.xyz, r3.xyz);
  r3.y = dot(r5.xyz, r4.xyz);
  r3.z = dot(r5.xyz, r2.xyz);
  r2.xyz = r10.xxx * r3.xyz;
  r4.xyzw = r3.yzxx * -r1.zxzx;
  r3.xyz = -r3.zxy * -r1.zzx + r4.xyz;
  r3.w = -r4.w;
  r2.w = 0;
  r2.xyzw = r3.xyzw + r2.xyzw;
  r3.xyz = r2.yzx * r1.zxz;
  r3.xyz = r1.zzx * r2.zxy + -r3.xyz;
  r2.xyz = r10.xxx * r2.xyz + r3.xyz;
  r1.xyz = r2.www * r1.xyz + r2.xyz;
  r2.xyz = cb1[1].xyz * r1.yyy;
  r2.xyz = cb1[0].xyz * r1.xxx + r2.xyz;
  o1.xyz = cb1[2].xyz * r1.zzz + r2.xyz;
  p1.x = v4.y;
  o3.xyzw = v0.xyzw;
  o4.x = v3.w;
  o4.yz = float2(0,0);
  o4.w = v4.x;
  r1.x = cmp(cb0[13].y < 9.99999997e-007);
  r1.x = (int)r1.x | (int)r1.w;
  o5.xyzw = r1.xxxx ? float4(10000,10000,-10000,1) : r0.xyzw;
  return;
}