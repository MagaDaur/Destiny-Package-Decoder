  ...
  out float4 o0 : SV_TARGET0)
{
  ...
  r0.x = exp2(r0.x);
  r0.y = 1 + -r0.x;
  r0.x = cb0[100].x * r0.y + r0.x;
  r0.x = 1 + -r0.x;
  r0.x = saturate(r0.x * -2.95989299 + 2.95989299);
  r0.xyzw = r0.xxxx * float4(-0.980068982,-0.980068982,-0.980068982,-1) + cb0[96].xyzw;
  r1.x = max(0.00100000005, v7.x);
  r1.x = log2(r1.x);
  r1.x = 2.20000005 * r1.x;
  r1.x = exp2(r1.x);
  r1.x = 1 + -r1.x;
  r1.y = max(9.99999975e-006, r1.x);
  r1.x = r1.x / r1.y;
  r1.x = max(0, r1.x);
  ...
  r2.x = cb0[108].x * v6.w;
  r2.x = floor(r2.x);
  r2.xy = cb0[105].xy * r2.xx;
  r2.xy = v0.xy * cb0[104].xy + r2.xy;
  r2.x = t0.Sample(s2_s, r2.xy).x;
  r2.x = log2(r2.x);
  r2.x = 2.20000005 * r2.x;
  r2.x = exp2(r2.x);
  r2.x = saturate(v7.z * r2.x);
  r1.w = saturate(v0.z * r2.x);
