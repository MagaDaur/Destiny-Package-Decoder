...
// unknown dcl_: dcl_input_sgv v4.x, vertex_id
  ...
  if (r0.x != 0) {
    ...
    r0.x = r0.w ? r1.y : r0.x;
    r1.y = (uint)r0.x << 2;
    bitmask.z = ((~(-1 << 30)) << 2) & 0xffffffff;  r1.z = (((uint)r0.x << 2) & bitmask.z) | ((uint)1 & ~bitmask.z);
    bitmask.w = ((~(-1 << 30)) << 2) & 0xffffffff;  r1.w = (((uint)r0.x << 2) & bitmask.w) | ((uint)2 & ~bitmask.w);
    if (r0.w != 0) {
      r0.x = (uint)r0.y << 2;
      r0.x = (int)r0.x & 1792;
      ...
      r3.z = cb1[r1.w+8].w;
      r0.w = (uint)r0.x << 2;
      bitmask.z = ((~(-1 << 30)) << 2) & 0xffffffff;  r2.z = (((uint)r0.x << 2) & bitmask.z) | ((uint)1 & ~bitmask.z);
      bitmask.w = ((~(-1 << 30)) << 2) & 0xffffffff;  r2.w = (((uint)r0.x << 2) & bitmask.w) | ((uint)2 & ~bitmask.w);
      ...
      if (r1.x != 0) {
        ...
        r6.zw = (uint2)r0.yw << int2(2,2);
        bitmask.x = ((~(-1 << 30)) << 2) & 0xffffffff;  r0.x = (((uint)r0.x << 2) & bitmask.x) | ((uint)1 & ~bitmask.x);
        bitmask.y = ((~(-1 << 30)) << 2) & 0xffffffff;  r0.y = (((uint)r0.y << 2) & bitmask.y) | ((uint)2 & ~bitmask.y);
        bitmask.z = ((~(-1 << 30)) << 2) & 0xffffffff;  r0.z = (((uint)r0.z << 2) & bitmask.z) | ((uint)1 & ~bitmask.z);
        bitmask.w = ((~(-1 << 30)) << 2) & 0xffffffff;  r0.w = (((uint)r0.w << 2) & bitmask.w) | ((uint)2 & ~bitmask.w);
    ...
    r4.z = dot(r2.xyz, r6.xyz);
    r1.w = dot(r4.xyz, r4.xyz);
    r1.w = rsqrt(r1.w);
    r2.xyzw = r4.xyzz * r1.wwww;
    r4.xyz = r2.ywx * r1.zxy;
    r4.xyz = r1.yzx * r2.wxy + -r4.xyz;
    o2.xyz = v2.www * r4.xyz;
    r4.xyzw = v3.xyxy * cb1[6].xyxy + cb1[6].zwzw;
    r5.xy = r4.zw * cb0[2].xy + cb0[2].zw;
    r1.w = t2.SampleLevel(s1_s, r5.xy, 0).x;
    r1.w = saturate(r1.w * cb0[3].x + cb0[3].y);
    r1.w = cb0[1].x * r1.w;
    r5.xyz = cb0[0].xyz * r1.www;
    r3.xyz = cb0[0].xyz * r1.www + r3.xyz;
    ...
    r5.xyz = cb0[6].xxx * r1.xyz;
    o4.xyz = r1.xyz * cb0[6].xxx + r3.xyz;
